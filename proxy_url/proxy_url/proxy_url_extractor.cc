...
namespace qh
{
    ...
    void ProxyURLExtractor::Extract( const KeyItems& keys, const std::string& raw_url, std::string& sub_url )
    {
		...
		/**
		*  Split the "key1=value1&key2=value2" pair through '&'
		*  Then split "key1=value1" through '='
		*  So, we can get key1 and value1
		*/
		...
		while (!token.isEnd()) {
			...
			if (key_value.empty()) {
				token.skipSpaces();  // ingore the whitespace
				const char* curpos = token.getCurReadPos();
				int nreadable = token.getReadableSize();
				if (token.skipTo('&') == 0) {  // string do not have '&', it means the last "key=value" pair
			...
			Tokener tokenTemp(key_value);  // "key=value"
			key = tokenTemp.nextString('=');
			if (keys.find(key) != keys.end()) {
				const char* curpos = tokenTemp.getCurReadPos();
				int nreadable = tokenTemp.getReadableSize();
